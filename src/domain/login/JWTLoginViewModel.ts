...
import type {JWTLoginMethod} from '../../matrix/login';
import {Options as BaseOptions, ViewModel} from '../ViewModel';
import type {LoginOptions} from './LoginViewModel';

type Options = {
    loginOptions: LoginOptions | undefined;
    attemptLogin: (loginMethod: JWTLoginMethod) => Promise<null>;
} & BaseOptions;

export class JWTLoginViewModel extends ViewModel {
    private _loginOptions?: LoginOptions;
    private _attemptLogin: (loginMethod: JWTLoginMethod) => Promise<null>;
    ...
    async login(loginToken: string): Promise<void> {
        this._errorMessage = '';
        this.emitChange('errorMessage');
        const status = await this._attemptLogin(this._loginOptions!.jwt!(loginToken));
        let error = '';
        switch (status) {
            ...
            case LoginFailure.Unknown:
                error = this.i18n`Something went wrong while checking your token.`;
                break;
        }
        if (error) {
